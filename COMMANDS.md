...
| `npm run deploy` | Builds (`npm run build`) then FTPS-syncs `out/` to Hostinger via `deploy.js` (uses `ftp-deploy-static`). Credentials come from `.env` (git-ignored) — copy `.env.example` and fill in `FTP_HOST` / `FTP_USER` / `FTP_PASSWORD` from hPanel → Files → FTP Accounts. `mode: 'sync'` uploads new/changed files and deletes stale ones; `protectedItems` in `deploy.js` lists server paths the sync must never touch. Prompts before uploading >20 MB. |
