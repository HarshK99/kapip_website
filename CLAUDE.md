...
**Brand ramp + gradient.** A separate decorative palette (`brand-indigo` `#1B2C74` / `brand-teal` `#14827A` / `brand-green` `#2E9E5B` / `brand-lime` `#AEC61C`, combined into `--brand-gradient`) exists for the KAP brand identity. It never substitutes for the functional `accent`/`accent-deep` pair, and it's scoped to four places: the signature mark/logo (gradient-filled — the one exception to the mark's usual flat `accent`), one hero accent moment, thin dividers/underlines, and `SectionHeading`'s big name text sitewide (`tone="brand"` on `ScrollColorHeading`). Still never as a background wash behind body paragraphs or a full section fill. These hex values are provisional until synced to the final logo — see `docs/DESIGN.md` → Palette.

**Auto-advancing interactions are allowed when paired with real pause control.** The default is still restrained, static interactions — but a component may auto-cycle (e.g. the Home services node explorer) if it pauses on hover/focus and stops entirely under `prefers-reduced-motion`, with no on-page instructional copy ("hover to pause" etc.) — the behavior should just work. See `docs/DESIGN.md` → Motion.
