...
**Two gradient variables — full vs. text-safe.** Measured WCAG contrast of each stop against `paper` (`FBFAF7`): `brand-indigo` 12.13:1, `brand-teal` 4.47:1, `brand-green` 3.27:1, `brand-lime` **1.85:1** — lime fails even the large-text 3:1 AA floor by a wide margin, and green only just clears it. So there are two CSS vars, not one:
...
- `--brand-gradient-text: linear-gradient(120deg, #1B2C74 0%, #14827A 45%, #2E9E5B 100%)` — indigo→teal→green only, lime dropped — for `SectionHeading`'s gradient-filled name text. Every remaining stop clears 3:1 against `paper` at the large/bold sizes headings render at. Do not reuse the plain 4-stop `--brand-gradient` for any text fill.
...
### Heading color
`SectionHeading`'s big name text renders in `--brand-gradient-text` (`ScrollColorHeading`'s `tone="brand"`) instead of flat `ink` — see Palette → Two gradient variables above for the contrast reasoning (lime dropped; the 3 remaining stops clear AA at heading sizes). This replaces the old scroll-linked color-wipe for these headings specifically: a static gradient fill reads more like a signature than an animated one would, and it avoids re-deriving a moving 2-color wipe into a 3-stop gradient. The wipe-on-scroll behavior (`tone="ink"`/`"paper"`) stays exactly as before for every heading that isn't going through `SectionHeading` (Home hero, deliver-item names, credibility/expertise h3s) — those are secondary text, not a section's name, and stay quiet.
